...
const siteUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://amaramoon.capetown'
...
# Force https and the bare domain in one hop. www.${siteHost} used to be the
# live Wix site, and Google had it indexed as a second, stale copy of the
# business (old URLs, old content) right alongside this one. Every request
# landing on one canonical host is what actually fixes that split, a hand
# edit on the server would just get overwritten by the next export anyway.
RewriteCond %{HTTPS} off [OR]
RewriteCond %{HTTP_HOST} ^www\\. [NC]
RewriteRule ^(.*)$ https://${siteHost}/$1 [R=301,L]

# The old Wix site's URLs, still sitting in Google's index and in old
# bookmarks/backlinks. Send each to its closest match on the new site instead
# of letting them 404, so the links and any ranking they built keep working.
RewriteRule ^about/?$                                                    /                     [R=301,L]
RewriteRule ^booking-engine/?$                                           /book-a-room/         [R=301,L]
RewriteRule ^service-page/sauna-40min-infrared/?$                        /offerings/sauna-40/  [R=301,L]
RewriteRule ^service-page/?(.*)$                                         /offerings/           [R=301,L]
RewriteRule ^post/explore-health-and-wellness-at-amara-moon-hout-bay/?$  /offerings/           [R=301,L]
RewriteRule ^post/?(.*)$                                                 /                     [R=301,L]
RewriteRule ^blog/?(.*)$                                                 /                     [R=301,L]

# Belt and suspenders for the trailing slash every canonical tag and the
# sitemap already assume. Apache's own mod_dir does this automatically on
# most hosts, but not all, and an explicit rule cannot regress on the ones
# where it is already implicit.
